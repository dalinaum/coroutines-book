...
### 코드 2-18 생성자와 속성 (constructor.kt)

클래스 이름 뒤에 파러미터 리스트를 붙이면 생성자가 된다.

클래스 생성자에서 var, val을 붙이면 해당 파라미터의 이름과 같은 속성이 만들어지고 대입된다.
...
### 코드 2-24 간단한 속성 (property.kt)

클래스 내에서 변수를 선언하듯 간단히 속성을 만들 수 있다.
...
### 코드 2-25 속성 세터 (propoerty-setter.kt)

`set(value)`로 속성에 세터를 설정할 수 있다. 게터는 자동으로 생성된 것을 사용한다.
...
### 코드 2-26 속성 게터 (property-getter.kt)

`get()`로 속성에 게터를 설정할 수 있다. 세터는 자동으로 생성된 것을 사용한다.
...
### 코드 2-27 뒷받침하는 속성 (backing-property.kt)

속성이 직접 필드를 가지지 않고 다른 속성에 의존하는 방법이다.

`list` 속성이 `var`인 경우에는 세터까지 설정해야 한다.
...
### 코드 2-29 위임된 속성 observable (observable.kt)

`kotlin.properties.Delegates.observable`을 이용하면 속성 값의 변경을 추적할 수 있다.
...
### 코드 2-30 다른 속성으로 위임 (delegate-to-other.kt)

다른 속성으로 위임할 수 있다. `@Deprecated` 등에 사용 가능.
...
### 코드 2-31 커스텀 위임된 속성 (custom-delegated-property.kt)
...
class CustomDelegate {
    ...
    operator fun getValue(thisRef: Any?, property: KProperty<*>): String {
        println("$thisRef, '${property.name}' 속성을 위임했습니다.")
...
#### 코드 2-44 범위 지정 함수 run (run.kt)

run은 with와 거의 같은데 확장 함수다.

<div class="kotlin-playground">
fun main() {
    val name = Person("홍길동", 20, "한양").run {
        println(this)
        moveTo("율도국")
        incrementAge()
        println(this)
...
#### 코드 2-45 확장 함수가 아닌 run (non-extension-run.kt)

확장 함수가 아닌 run도 있다. 이 함수는 현재 문맥을 가지지 않고 람다를 실행해서 결과를 리턴한다.
...
fun main() {
    ...
    println("현재 날짜는 ${date}입니다.")
}
</div>

### 2.11.4 범위 지정 함수 apply

#### 코드 2-46 범위 지정 함수 appy (apply.kt)

`apply`는 `T`에 대한 확장 함수로 `T`에 대한 수신 객체 지정 람다를 파라미터로 받는다. 반환 값은 수신 객체 `T`가 반환 값으로 람다의 반환 값이 아니다.

`apply`는 주로 설정을 위해 사용하는 경향이 있다.

<div class="kotlin-playground">
fun main() {
    val person = Person("홍길동", 20, "한양").apply {
        println(this)
        age = 21
        city = "율도국"
    }
...
### 2.11.5 범위 지정 함수 also

#### 코드 2-47 범위 지정 함수 also (also.kt)

`also`는 `T`에 대한 확장 함수다. `T`를 파라미터로 받는 람다를가 사용된다. 반환 값은 수신 객체 `T`가 반환 값으로 람다의 반환 값이 아니다.

`also`는 주로 같은 객체를 이용해 연속적인 함수 호출 등을 할 때 사용한다.

<div class="kotlin-playground">
fun main() {
    val person = Person("hong gildong", 20, "hanyang").also {
        println("${it.name.uppercase()}")
        println("${it.city.capitalize()}")
        it.incrementAge()
    }
    println(person)
}
...
### 코드 2-48 비구조화 (destructure.kt)

구조화(Structuring)된 항목에서 특정 항목을 가져올 수 있는 비구조화(Destructuring)이 가능하다.

`person`에서 `name`, `age`, `city`를 가져오자.

<div class="kotlin-playground">
fun main() {
    val person = Person("홍길동", 20, "한양")
...
### 코드 2-49 개수가 부족한 비구조화 (destructure2.kt)

개수를 부족하게 가져오면 앞에서부터 가져온다.
...
### 코드 2-50 일부를 버리는 비구조화 (destructure3.kt)

`_`를 이용하면 일부를 버릴 수 있다.
...
### 코드 2-51 component 메서드 (component.kt)

데이터 클래스는 속성의 개수만큼 `componentXXX` 메서드를 만든다. 이 메서드는 비구조화에서 활용된다.
...
### 코드 2-52 람다에서 비구조화 (lambda-destructure.kt)

람다의 파라미터에서도 비구조화를 사용할 수 있다.

<div class="kotlin-playground">
fun main() {
    handlePerson { (name, age, city) ->
        println("name: $name, age: $age, city: $city")
    }
}

fun handlePerson(handler: (Person)->Unit) {
    val person = Person("홍길동", 20, "한양")
    handler(person)
}
...
### 코드 2-53 반환 값에서 비구조화 (return-value-destructure.kt)

반환 값에서도 비구조화를 사용할 수 있다.

<div class="kotlin-playground">
fun main() {
    findProperPerson().let { (name, age, city) ->
        println("name: $name, age: $age, city: $city")
    }
}

fun findProperPerson() = Person("홍길동", 20, "한양")
...
### 코드 2-54 데이터 클래스가 만들어준 메서드 (data-class-methods.kt)

데이터 클래스는 `hashCode`, `equals`, `componentXXX`, `copy`, `toString` 등의 메서드를 자동 생성한다.

<div class="kotlin-playground">
fun main() {
    val person = Person("홍길동", 20, "한양")
    val person2 = Person("홍길동", 20, "한양")
    ...
    val person3 = person2.copy(name="호부호형맨")
    println(person)
    println(person2)
    println(person3)
}
...
### 코드 2-55 연산자 오버로딩 (operator.kt)

연산자 오버로딩을 할 때는 `operator` 키워드를 사용한다.

`[]`는 `get` 메서드로 `-`는 `unaryMinus` 메서드로 구현할 수 있다.
...
operator fun Person.unaryMinus() = this.copy(age = -age)

operator fun Person.get(index: Int) = this.copy(name = "홍길동 $index")

data class Person(var name: String, var age: Int, var city: String) {
    operator fun unaryPlus() = this.copy(name = "+$name")
}
</div>

### 코드 2-56 단항 마이너스 연산자 오버로딩 (unary-minus.kt)

`Point` 데이터 클래스를 만들고 단항 마이너스 연산자를 구현했다.

<div class="kotlin-playground">
fun main() {
    val point = Point(30, 40)
    println(point)
    println(-point)
}
...
### 코드 2-57 Point 다양한 연산자 오버로딩 (point-with-operators.kt)

`Point` 데이터 클래스에 `-`, `++`, `-`, `+` 연산자를 구현한다.
...
data class Point(val x: Int, val y: Int) {
    ...
    operator fun minus(n: Int) = Point(x - n, y - n)
